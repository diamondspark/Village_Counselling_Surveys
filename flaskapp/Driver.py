...
def sort_ph1(test_case_removal):
    ...
    writer = pd.ExcelWriter('/var/www/html/flaskapp/Output/Phase1 '+current_date+'.xlsx')
    output.df.to_excel(writer,startcol = 0, startrow = 5,index=False)
...
def sort_ph2(test_case_removal):
    ...
    writer = pd.ExcelWriter('/var/www/html/flaskapp/Output/Phase2 '+current_date+'.xlsx')
    ...
    worksheet.write_string(5, 0, "Total Phase2 Participants ")
    worksheet.write_string(5, 1, str(total_participants))
    worksheet.write_string(6, 0, "Average of before scores ")
